...
if horizontalFlag
    hReg2(end+1) = rectangle('Position',[boxEdge(1),y-barWidth/2,IQR+1e-8,barWidth],'linewidth',linewidth,'EdgeColor',boxColor);
    ...
    hReg2(end+1) = rectangle('Position',[y-barWidth/2,boxEdge(1),barWidth,IQR+1e-8],'linewidth',linewidth,'EdgeColor',boxColor);
    hold on;
    
    hReg2(end+1) = plot([y-barWidth/2 y+barWidth/2],[medianX medianX],'color',boxColor,'linewidth',linewidth);
    if meanFlag
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(xx)
        if horizontalFlag
            hReg2(6) = plot(xx,yy,'o','linewidth',linewidth,'color',outColor);
        else
             hReg2(6) = plot(yy,xx,'o','linewidth',linewidth,'color',outColor);
        end
    end
end
